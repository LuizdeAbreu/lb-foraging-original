...
class DQNAgent(Agent):
    ...
    def save(self, path, agent_number):
        # print("Saving DQN agent to saved/policy_{}.pt and saved/target_{}.pt".format(path, path))
        torch.save(self.policy_net.state_dict(), "{}/policy_{}.pt".format(path, agent_number))
        torch.save(self.target_net.state_dict(), "{}/target_{}.pt".format(path, agent_number))

        torch.save({
            'optimizer_state_dict': self.optimizer.state_dict(),
            'steps_done': self.steps_done,
            'memory': self.memory,
        }, "{}/optimizer.pt".format(path))

    def load(self, path):
        print("Loading DQN agent from saved/policy_{}.pt and saved/target_{}.pt".format(path, path))
        self.policy_net.load_state_dict(torch.load("saved/policy.pt".format(path)))
        self.target_net.load_state_dict(torch.load("saved/target.pt".format(path)))

        checkpoint = torch.load("saved/optimizer.pt".format(path))
